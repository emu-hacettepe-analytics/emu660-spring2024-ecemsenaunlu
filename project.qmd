...
```
The correlation coefficient took the value of 0.9178788, indicating that there is a very strong positive relationship between these two variables.
The p-value is 0.00018, indicating that this correlation is statistically significant. The confidence interval is quite narrow, indicating the accuracy of the correlation.
As a result, we can say that there is a very strong and statistically significant positive correlation between DoctoratePercentage and GDP contribution rate variables. This shows that GDP tends to increase as the percentage of PhDs increases.
...
In conclusion;
 ...
 4.2. According to the 5-year time series analysis conducted with the second data set, which includes the distribution of crime rates over years by education levels, it was observed that the relevant data was compatible with the model and that education levels were also related to crime rates. When we look at the 5-year analysis, it is concluded that the crime rate has stabilized.
